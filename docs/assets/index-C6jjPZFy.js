...
`)}},$K=class{version;objSvg;morphSvg;panel;runBtn;closeBtn;progressEl;progressStage;progressBar;resultsEl;constructor(e,t,n){this.version=e,this.objSvg=t,this.morphSvg=n,this.panel=b(`#perf-panel`),this.runBtn=b(`#perf-run-btn`),this.closeBtn=b(`#perf-close`),this.progressEl=b(`#perf-progress`),this.progressStage=b(`#perf-progress-stage`),this.progressBar=b(`#perf-progress-bar`),this.resultsEl=b(`#perf-results`),this.bindEvents()}bindEvents(){this.runBtn.addEventListener(`click`,()=>this.runTests()),this.closeBtn.addEventListener(`click`,()=>this.hide()),window.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key.toLowerCase()===`p`&&(e.preventDefault(),this.toggle())})}toggle(){this.panel.classList.contains(`hidden`)?this.show():this.hide()}show(){this.panel.classList.remove(`hidden`)}hide(){this.panel.classList.add(`hidden`)}async runTests(){this.runBtn.disabled=!0,this.resultsEl.textContent=``,this.progressEl.style.display=`block`;let e=await new QK(this.version).run(this.objSvg,this.morphSvg,(e,t)=>{this.progressStage.textContent=e,this.progressBar.style.width=`${t}%`}),t=QK.formatReport(e);this.resultsEl.textContent=t,console.log(t);try{await navigator.clipboard.writeText(t),this.progressStage.textContent=`✓ Скопировано в буфер обмена`}catch{this.progressStage.textContent=`Результаты в панели — скопируй вручную`}this.runBtn.disabled=!1}},eq=[`#FFD93D`,`#FF6B9D`,`#FF8C69`,`#C3B1E1`],tq=[`#8a5c00`,`#8a1040`,`#a03010`,`#5a3e8a`],nq=[`Правильно!`,`Да!`,`Отлично!`,`Супер!`,`Молодец!`],rq=[`Почти!`,`Близко!`,`Попробуй ещё!`,`Хорошая попытка!`],iq=10,aq=class{menuScreen=b(`#menu-screen`);gameScreen=b(`#game-screen`);completeScreen=b(`#complete-screen`);bgSvg=b(`#bg-svg`);objSvg=b(`#obj-svg`);morphSvg=b(`#morph-svg`);questionCard=b(`.card`);qText=b(`#q-text`);scoreEl=b(`#score-val`);qNumEl=b(`#q-num`);feedback=b(`#feedback`);nextBtn=b(`#next-btn`);answersWrap=b(`#answers`);drawWrap=b(`#draw-wrap`);drawCheckBtn=b(`#draw-check-btn`);drawClearBtn=b(`#draw-clear-btn`);finalTxt=b(`#final-txt`);timerTrack=b(`#timer-track`);timerBar=b(`#timer-bar`);mouseTracker=new T;scrollTransition;timer;questionGen;morpher;drawingPad=null;state={phase:`menu`,score:0,questionIndex:0,totalQuestions:iq,answered:!1,difficulty:`easy`};currentAnswer=0;morphLoopActive=!1;constructor(){new E(this.bgSvg,this.mouseTracker),new D,this.scrollTransition=new ee(this.questionCard),this.timer=new ne(this.timerBar),this.questionGen=new C(this.objSvg),this.morpher=new ce(this.morphSvg),this.buildStars(),this.bindMenuEvents(),this.bindGameEvents(),this.startMenuMorphLoop(),new $K(`CSS version`,this.objSvg,this.morphSvg)}async startMenuMorphLoop(){for(this.morphLoopActive=!0;this.morphLoopActive;)await this.morpher.morphToRandom(),await y(1200)}stopMenuMorphLoop(){this.morphLoopActive=!1}bindMenuEvents(){document.querySelectorAll(`.diff-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.diff;this.startGame(t)}),e.addEventListener(`mouseenter`,()=>{e.style.transform=`scale(1.08) rotate(-2deg)`}),e.addEventListener(`mouseleave`,()=>{e.style.transform=``})})}async startGame(e){this.stopMenuMorphLoop(),e===`drawing`&&await this.ensureRecognizerReady(),this.state={phase:`playing`,score:0,questionIndex:0,totalQuestions:iq,answered:!1,difficulty:e},this.scrollTransition.enabled=!0,this.menuScreen.classList.add(`hidden`),this.gameScreen.classList.remove(`hidden`),this.completeScreen.classList.add(`hidden`),this.loadQuestion()}async ensureRecognizerReady(){let e=b(`#loading-overlay`),t=b(`#loading-stage`),n=b(`#loading-pct`),r=b(`#loading-bar`);e.classList.remove(`hidden`),await XK.initRecognizer((e,i)=>{t.textContent=e,n.textContent=`${Math.round(i)}%`,r.style.width=`${i}%`}),await new Promise(e=>setTimeout(e,400)),e.classList.add(`hidden`)}bindGameEvents(){this.nextBtn.addEventListener(`click`,()=>this.advanceQuestion()),this.scrollTransition.onNext(()=>{this.state.answered&&this.advanceQuestion()}),this.drawCheckBtn.addEventListener(`click`,()=>{this.drawingPad?.recognize()}),this.drawClearBtn.addEventListener(`click`,()=>{this.drawingPad?.clear()})}loadQuestion(){this.state.answered=!1,this.feedback.textContent=``,this.feedback.className=`feedback`,this.nextBtn.style.display=`none`,this.answersWrap.innerHTML=``;let e=S[this.state.difficulty],t=this.questionGen.generate(this.state.questionIndex,this.state.difficulty);this.currentAnswer=t.answer,this.qText.textContent=t.text,this.qNumEl.textContent=String(this.state.questionIndex+1),this.scoreEl.textContent=String(this.state.score),e.drawingMode?this.showDrawingMode():(this.showButtonMode(),this.buildAnswerButtons(t.answer)),e.timeLimit?(this.timerTrack.style.display=`block`,this.timer.start(e.timeLimit,()=>this.onTimeUp())):this.timerTrack.style.display=`none`,this.morpher.morphToRandom()}showDrawingMode(){this.answersWrap.style.display=`none`,this.drawWrap.style.display=`flex`,this.drawingPad&&this.drawingPad.destroy();let e=b(`#drawing-pad-container`);e.innerHTML=``,this.drawingPad=new XK(e),window.collect=e=>this.drawingPad?.collectSample(e),window.downloadDataset=()=>XK.downloadDataset(),this.drawingPad.setCorrectAnswer(this.currentAnswer),this.drawingPad.onRecognized(({digit:e,score:t,correct:n})=>{if(!this.state.answered)if(n)this.state.answered=!0,this.state.score++,this.scoreEl.textContent=String(this.state.score),this.feedback.textContent=nq[v(0,nq.length-1)],this.feedback.className=`feedback correct`,te(),this.nextBtn.style.display=`inline-block`;else{let n=Math.round(t*100);this.feedback.textContent=e>=0?`Похоже на ${e} (${n}%) — попробуй ещё!`:`Не разобрать — нарисуй чище!`,this.feedback.className=`feedback wrong`}})}showButtonMode(){this.answersWrap.style.display=`flex`,this.drawWrap.style.display=`none`,this.drawingPad&&=(this.drawingPad.destroy(),null)}buildAnswerButtons(e){w(e).forEach((t,n)=>{let r=document.createElement(`button`);r.className=`ans-btn`,r.dataset.v=String(t);let i=eq[n],a=tq[n];r.innerHTML=`<svg viewBox="0 0 90 90" xmlns="http://www.w3.org/2000/svg">
      ...
      </svg>`,r.addEventListener(`mouseenter`,()=>{r.style.transform=`scale(1.12) translateY(-4px)`,r.style.filter=`drop-shadow(0 8px 16px ${i}88)`}),r.addEventListener(`mouseleave`,()=>{r.style.transform=``,r.style.filter=``}),r.addEventListener(`click`,()=>this.onAnswer(t,e,r)),this.answersWrap.appendChild(r)})}onAnswer(e,t,n){this.state.answered||(this.state.answered=!0,this.timer.stop(),e===t?(this.state.score++,this.scoreEl.textContent=String(this.state.score),this.feedback.textContent=nq[v(0,nq.length-1)],this.feedback.className=`feedback correct`,te()):(this.feedback.textContent=rq[v(0,rq.length-1)],this.feedback.className=`feedback wrong`,this.highlightCorrect(t)),this.nextBtn.style.display=`inline-block`)}onTimeUp(){this.state.answered||(this.state.answered=!0,this.feedback.textContent=`Время вышло!`,this.feedback.className=`feedback wrong`,this.highlightCorrect(this.currentAnswer),this.nextBtn.style.display=`inline-block`)}highlightCorrect(e){document.querySelectorAll(`.ans-btn`).forEach(t=>{if(Number(t.dataset.v)===e){let e=t.querySelector(`rect`),n=t.querySelector(`text`),r=t.querySelector(`svg`);e.setAttribute(`fill`,`#00e676`),e.setAttribute(`stroke`,`#ffffff`),e.setAttribute(`stroke-width`,`5`),n.setAttribute(`fill`,`#004d20`),r.style.filter=`drop-shadow(0 0 10px #00e676) drop-shadow(0 0 20px #00e676)`,r.style.animation=`correctPulse 0.5s ease-in-out infinite alternate`}})}async advanceQuestion(){if(this.state.questionIndex++,this.state.questionIndex>=iq){this.showComplete();return}await this.scrollTransition.transition(()=>this.loadQuestion())}showComplete(){this.state.phase=`complete`,this.scrollTransition.enabled=!1,this.timer.hide(),this.showButtonMode(),this.gameScreen.classList.add(`hidden`),this.completeScreen.classList.remove(`hidden`),this.finalTxt.textContent=`Правильно: ${this.state.score} из ${iq}!`,b(`#restart-btn`).onclick=()=>this.startGame(this.state.difficulty),b(`#newgame-btn`).onclick=()=>{this.completeScreen.classList.add(`hidden`),this.menuScreen.classList.remove(`hidden`),this.startMenuMorphLoop()}}buildStars(){let e=`http://www.w3.org/2000/svg`,t=this.bgSvg.querySelector(`#star-g`);for(let n=0;n<90;n++){let n=document.createElementNS(e,`circle`);n.setAttribute(`cx`,String(Math.random()*1e3)),n.setAttribute(`cy`,String(Math.random()*700)),n.setAttribute(`r`,String(Math.random()*2+.4)),n.setAttribute(`fill`,`white`);let r=document.createElementNS(e,`animate`);r.setAttribute(`attributeName`,`opacity`),r.setAttribute(`values`,`1;0.05;1`),r.setAttribute(`dur`,`${(Math.random()*3+1.5).toFixed(1)}s`),r.setAttribute(`begin`,`${(Math.random()*4).toFixed(1)}s`),r.setAttribute(`repeatCount`,`indefinite`),n.appendChild(r),t.appendChild(n)}}},oq=`mathgame-collected-digits`,sq=class{pad;preprocessor=new YK;counterEl;static isRequested(){return new URLSearchParams(window.location.search).has(`collect`)}constructor(){document.body.innerHTML=``;let e=document.createElement(`div`);e.id=`collect-screen`,e.innerHTML=`
    ...
    `,document.body.appendChild(e);let t=e.querySelector(`.collect-digits`);for(let e=0;e<=9;e++){let n=document.createElement(`button`);n.className=`collect-digit-btn`,n.textContent=String(e),n.addEventListener(`click`,()=>this.save(e)),t.appendChild(n)}this.counterEl=e.querySelector(`#collect-counter`),this.pad=new XK(e.querySelector(`#collect-pad`)),e.querySelector(`#collect-clear`).addEventListener(`click`,()=>this.pad.clear()),e.querySelector(`#collect-download`).addEventListener(`click`,()=>this.download()),e.querySelector(`#collect-reset`).addEventListener(`click`,()=>{confirm(`Удалить все собранные примеры с этого устройства?`)&&(localStorage.removeItem(oq),this.updateCounter())}),this.updateCounter()}load(){try{return JSON.parse(localStorage.getItem(oq)??`[]`)}catch{return[]}}save(e){if(!this.pad.hasDrawing()){alert(`Сначала нарисуй цифру!`);return}let t=this.preprocessor.process(this.pad.getSvg()),n=Array.from(t,e=>Math.round(e*255)),r=this.load();r.push({label:e,pixels:n});try{localStorage.setItem(oq,JSON.stringify(r))}catch{alert(`Хранилище переполнено — скачай файл и нажми «Сбросить всё»`);return}this.pad.clear(),this.updateCounter()}updateCounter(){let e=this.load(),t=Array(10).fill(0);e.forEach(e=>t[e.label]++),this.counterEl.textContent=`Всего: ${e.length}  [`+t.map((e,t)=>`${t}:${e}`).join(` `)+`]`}download(){let e=this.load();if(e.length===0){alert(`Пока нет примеров`);return}let t=new Blob([JSON.stringify(e)],{type:`application/json`}),n=document.createElement(`a`);n.href=URL.createObjectURL(t),n.download=`user-digits-${e.length}.json`,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(n.href)}};document.addEventListener(`DOMContentLoaded`,()=>{sq.isRequested()?new sq:new aq});