...
`)}},QK=class{tracker=new ZK;version;constructor(e){this.version=e}async run(e,t,n){let r=[];n?.(`Тест 1: рендеринг фигур`,0),r.push(await this.testShapeRendering(e,n)),await $K(500),n?.(`Тест 2: морфинг`,33),r.push(await this.testMorphing(t,n)),await $K(500),n?.(`Тест 3: фоновая активность`,66),r.push(await this.testIdleAnimation()),n?.(`Готово`,100);let i=r.reduce((e,t)=>e+t.durationMs,0),a=r.map(e=>e.frameMetrics?.avgFps??0).filter(e=>e>0),o=a.reduce((e,t)=>e+t,0)/Math.max(a.length,1),s=r.reduce((e,t)=>e+(t.frameMetrics?.longFrames??0),0);return{version:this.version,timestamp:new Date().toISOString(),userAgent:navigator.userAgent,results:r,summary:{totalDurationMs:i,avgFpsAcrossTests:o,totalLongFrames:s}}}async testShapeRendering(e,t){return this.tracker.benchmark(`Рендеринг 50×9 фигур с анимациями`,async()=>{for(let n=0;n<50;n++){let r=u[n%u.length];_(e,9,r),await $K(50),n%5==0&&t?.(`Тест 1: рендеринг фигур (${n}/50)`,n/50*33)}})}async testMorphing(e,t){e.innerHTML=``;let n=new ce(e);return this.tracker.benchmark(`20 морфингов подряд`,async()=>{for(let e=0;e<20;e++)await n.morphToRandom(),t?.(`Тест 2: морфинг (${e+1}/20)`,33+(e+1)/20*33);n.destroy()})}async testIdleAnimation(){return this.tracker.benchmark(`5 секунд фоновой анимации`,async()=>{await $K(5e3)})}static formatReport(e){let t=[];return t.push(`═`.repeat(60)),t.push(`  ОТЧЁТ О ПРОИЗВОДИТЕЛЬНОСТИ — ${e.version}`),t.push(`═`.repeat(60)),t.push(`Дата:        ${e.timestamp}`),t.push(`Браузер:     ${e.userAgent}`),t.push(``),e.results.forEach(e=>{t.push(ZK.format(e)),t.push(``)}),t.push(`─`.repeat(60)),t.push(`  ИТОГ`),t.push(`─`.repeat(60)),t.push(`Общее время:     ${(e.summary.totalDurationMs/1e3).toFixed(2)}с`),t.push(`Средний FPS:     ${e.summary.avgFpsAcrossTests.toFixed(1)}`),t.push(`Долгих кадров:   ${e.summary.totalLongFrames}`),t.push(`═`.repeat(60)),t.join(`
`)}};function $K(e){return new Promise(t=>setTimeout(t,e))}var eq=class{version;objSvg;morphSvg;panel;runBtn;closeBtn;progressEl;progressStage;progressBar;resultsEl;constructor(e,t,n){this.version=e,this.objSvg=t,this.morphSvg=n,this.panel=tq(`#perf-panel`),this.runBtn=tq(`#perf-run-btn`),this.closeBtn=tq(`#perf-close`),this.progressEl=tq(`#perf-progress`),this.progressStage=tq(`#perf-progress-stage`),this.progressBar=tq(`#perf-progress-bar`),this.resultsEl=tq(`#perf-results`),this.bindEvents()}bindEvents(){this.runBtn.addEventListener(`click`,()=>this.runTests()),this.closeBtn.addEventListener(`click`,()=>this.hide()),window.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key.toLowerCase()===`p`&&(e.preventDefault(),this.toggle())})}toggle(){this.panel.classList.contains(`hidden`)?this.show():this.hide()}show(){this.panel.classList.remove(`hidden`)}hide(){this.panel.classList.add(`hidden`)}async runTests(){this.runBtn.disabled=!0,this.resultsEl.textContent=``,this.progressEl.style.display=`block`;let e=await new QK(this.version).run(this.objSvg,this.morphSvg,(e,t)=>{this.progressStage.textContent=e,this.progressBar.style.width=`${t}%`}),t=QK.formatReport(e);this.resultsEl.textContent=t,console.log(t);try{await navigator.clipboard.writeText(t),this.progressStage.textContent=`✓ Скопировано в буфер обмена`}catch{this.progressStage.textContent=`Результаты в панели — скопируй вручную`}this.runBtn.disabled=!1}};function tq(e){let t=document.querySelector(e);if(!t)throw Error(`Not found: ${e}`);return t}var nq=[`#FFD93D`,`#FF6B9D`,`#FF8C69`,`#C3B1E1`],rq=[`#8a5c00`,`#8a1040`,`#a03010`,`#5a3e8a`],iq=[`Правильно!`,`Да!`,`Отлично!`,`Супер!`,`Молодец!`],aq=[`Почти!`,`Близко!`,`Попробуй ещё!`,`Хорошая попытка!`],oq=10,sq=class{menuScreen=cq(`#menu-screen`);gameScreen=cq(`#game-screen`);completeScreen=cq(`#complete-screen`);bgSvg=cq(`#bg-svg`);objSvg=cq(`#obj-svg`);morphSvg=cq(`#morph-svg`);questionCard=cq(`.card`);qText=cq(`#q-text`);scoreEl=cq(`#score-val`);qNumEl=cq(`#q-num`);feedback=cq(`#feedback`);nextBtn=cq(`#next-btn`);answersWrap=cq(`#answers`);drawWrap=cq(`#draw-wrap`);drawCheckBtn=cq(`#draw-check-btn`);drawClearBtn=cq(`#draw-clear-btn`);finalTxt=cq(`#final-txt`);timerTrack=cq(`#timer-track`);timerBar=cq(`#timer-bar`);mouseTracker=new C;scrollTransition;timer;questionGen;morpher;drawingPad=null;state={phase:`menu`,score:0,questionIndex:0,totalQuestions:oq,answered:!1,difficulty:`easy`};currentAnswer=0;morphLoopActive=!1;constructor(){new w(this.bgSvg,this.mouseTracker),new T,this.scrollTransition=new E(this.questionCard),this.timer=new ee(this.timerBar),this.questionGen=new y(this.objSvg),this.morpher=new ce(this.morphSvg),this.buildStars(),this.bindMenuEvents(),this.bindGameEvents(),this.startMenuMorphLoop(),new eq(`CSS version`,this.objSvg,this.morphSvg)}async startMenuMorphLoop(){for(this.morphLoopActive=!0;this.morphLoopActive;)await this.morpher.morphToRandom(),await uq(1200)}stopMenuMorphLoop(){this.morphLoopActive=!1}bindMenuEvents(){document.querySelectorAll(`.diff-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.diff;this.startGame(t)}),e.addEventListener(`mouseenter`,()=>{e.style.transform=`scale(1.08) rotate(-2deg)`}),e.addEventListener(`mouseleave`,()=>{e.style.transform=``})})}async startGame(e){this.stopMenuMorphLoop(),e===`drawing`&&await this.ensureRecognizerReady(),this.state={phase:`playing`,score:0,questionIndex:0,totalQuestions:oq,answered:!1,difficulty:e},this.scrollTransition.enabled=!0,this.menuScreen.classList.add(`hidden`),this.gameScreen.classList.remove(`hidden`),this.completeScreen.classList.add(`hidden`),this.loadQuestion()}async ensureRecognizerReady(){let e=cq(`#loading-overlay`),t=cq(`#loading-stage`),n=cq(`#loading-pct`),r=cq(`#loading-bar`);e.classList.remove(`hidden`),await XK.initRecognizer((e,i)=>{t.textContent=e,n.textContent=`${Math.round(i)}%`,r.style.width=`${i}%`}),await new Promise(e=>setTimeout(e,400)),e.classList.add(`hidden`)}bindGameEvents(){this.nextBtn.addEventListener(`click`,()=>this.advanceQuestion()),this.scrollTransition.onNext(()=>{this.state.answered&&this.advanceQuestion()}),this.drawCheckBtn.addEventListener(`click`,()=>{this.drawingPad?.recognize()}),this.drawClearBtn.addEventListener(`click`,()=>{this.drawingPad?.clear()})}loadQuestion(){this.state.answered=!1,this.feedback.textContent=``,this.feedback.className=`feedback`,this.nextBtn.style.display=`none`,this.answersWrap.innerHTML=``;let e=v[this.state.difficulty],t=this.questionGen.generate(this.state.questionIndex,this.state.difficulty);this.currentAnswer=t.answer,this.qText.textContent=t.text,this.qNumEl.textContent=String(this.state.questionIndex+1),this.scoreEl.textContent=String(this.state.score),e.drawingMode?this.showDrawingMode():(this.showButtonMode(),this.buildAnswerButtons(t.answer)),e.timeLimit?(this.timerTrack.style.display=`block`,this.timer.start(e.timeLimit,()=>this.onTimeUp())):this.timerTrack.style.display=`none`,this.morpher.morphToRandom()}showDrawingMode(){this.answersWrap.style.display=`none`,this.drawWrap.style.display=`flex`,this.drawingPad&&this.drawingPad.destroy();let e=cq(`#drawing-pad-container`);e.innerHTML=``,this.drawingPad=new XK(e),window.collect=e=>this.drawingPad?.collectSample(e),window.downloadDataset=()=>XK.downloadDataset(),this.drawingPad.setCorrectAnswer(this.currentAnswer),this.drawingPad.onRecognized(({digit:e,score:t,correct:n})=>{if(!this.state.answered)if(n)this.state.answered=!0,this.state.score++,this.scoreEl.textContent=String(this.state.score),this.feedback.textContent=iq[lq(0,iq.length-1)],this.feedback.className=`feedback correct`,D(),this.nextBtn.style.display=`inline-block`;else{let n=Math.round(t*100);this.feedback.textContent=e>=0?`Похоже на ${e} (${n}%) — попробуй ещё!`:`Не разобрать — нарисуй чище!`,this.feedback.className=`feedback wrong`}})}showButtonMode(){this.answersWrap.style.display=`flex`,this.drawWrap.style.display=`none`,this.drawingPad&&=(this.drawingPad.destroy(),null)}buildAnswerButtons(e){b(e).forEach((t,n)=>{let r=document.createElement(`button`);r.className=`ans-btn`,r.dataset.v=String(t);let i=nq[n],a=rq[n];r.innerHTML=`<svg viewBox="0 0 90 90" xmlns="http://www.w3.org/2000/svg">
      ...
      </svg>`,r.addEventListener(`mouseenter`,()=>{r.style.transform=`scale(1.12) translateY(-4px)`,r.style.filter=`drop-shadow(0 8px 16px ${i}88)`}),r.addEventListener(`mouseleave`,()=>{r.style.transform=``,r.style.filter=``}),r.addEventListener(`click`,()=>this.onAnswer(t,e,r)),this.answersWrap.appendChild(r)})}onAnswer(e,t,n){this.state.answered||(this.state.answered=!0,this.timer.stop(),e===t?(this.state.score++,this.scoreEl.textContent=String(this.state.score),this.feedback.textContent=iq[lq(0,iq.length-1)],this.feedback.className=`feedback correct`,D()):(this.feedback.textContent=aq[lq(0,aq.length-1)],this.feedback.className=`feedback wrong`,this.highlightCorrect(t)),this.nextBtn.style.display=`inline-block`)}onTimeUp(){this.state.answered||(this.state.answered=!0,this.feedback.textContent=`Время вышло!`,this.feedback.className=`feedback wrong`,this.highlightCorrect(this.currentAnswer),this.nextBtn.style.display=`inline-block`)}highlightCorrect(e){document.querySelectorAll(`.ans-btn`).forEach(t=>{if(Number(t.dataset.v)===e){let e=t.querySelector(`rect`),n=t.querySelector(`text`),r=t.querySelector(`svg`);e.setAttribute(`fill`,`#00e676`),e.setAttribute(`stroke`,`#ffffff`),e.setAttribute(`stroke-width`,`5`),n.setAttribute(`fill`,`#004d20`),r.style.filter=`drop-shadow(0 0 10px #00e676) drop-shadow(0 0 20px #00e676)`,r.style.animation=`correctPulse 0.5s ease-in-out infinite alternate`}})}async advanceQuestion(){if(this.state.questionIndex++,this.state.questionIndex>=oq){this.showComplete();return}await this.scrollTransition.transition(()=>this.loadQuestion())}showComplete(){this.state.phase=`complete`,this.scrollTransition.enabled=!1,this.timer.hide(),this.showButtonMode(),this.gameScreen.classList.add(`hidden`),this.completeScreen.classList.remove(`hidden`),this.finalTxt.textContent=`Правильно: ${this.state.score} из ${oq}!`,cq(`#restart-btn`).onclick=()=>this.startGame(this.state.difficulty),cq(`#newgame-btn`).onclick=()=>{this.completeScreen.classList.add(`hidden`),this.menuScreen.classList.remove(`hidden`),this.startMenuMorphLoop()}}buildStars(){let e=`http://www.w3.org/2000/svg`,t=this.bgSvg.querySelector(`#star-g`);for(let n=0;n<90;n++){let n=document.createElementNS(e,`circle`);n.setAttribute(`cx`,String(Math.random()*1e3)),n.setAttribute(`cy`,String(Math.random()*700)),n.setAttribute(`r`,String(Math.random()*2+.4)),n.setAttribute(`fill`,`white`);let r=document.createElementNS(e,`animate`);r.setAttribute(`attributeName`,`opacity`),r.setAttribute(`values`,`1;0.05;1`),r.setAttribute(`dur`,`${(Math.random()*3+1.5).toFixed(1)}s`),r.setAttribute(`begin`,`${(Math.random()*4).toFixed(1)}s`),r.setAttribute(`repeatCount`,`indefinite`),n.appendChild(r),t.appendChild(n)}}};function cq(e){let t=document.querySelector(e);if(!t)throw Error(`Not found: ${e}`);return t}function lq(e,t){return Math.floor(Math.random()*(t-e+1))+e}function uq(e){return new Promise(t=>setTimeout(t,e))}var dq=`mathgame-collected-digits`,fq=class{pad;preprocessor=new YK;counterEl;static isRequested(){return new URLSearchParams(window.location.search).has(`collect`)}constructor(){document.body.innerHTML=``;let e=document.createElement(`div`);e.id=`collect-screen`,e.innerHTML=`
    ...
    `,document.body.appendChild(e);let t=e.querySelector(`.collect-digits`);for(let e=0;e<=9;e++){let n=document.createElement(`button`);n.className=`collect-digit-btn`,n.textContent=String(e),n.addEventListener(`click`,()=>this.save(e)),t.appendChild(n)}this.counterEl=e.querySelector(`#collect-counter`),this.pad=new XK(e.querySelector(`#collect-pad`)),e.querySelector(`#collect-clear`).addEventListener(`click`,()=>this.pad.clear()),e.querySelector(`#collect-download`).addEventListener(`click`,()=>this.download()),e.querySelector(`#collect-reset`).addEventListener(`click`,()=>{confirm(`Удалить все собранные примеры с этого устройства?`)&&(localStorage.removeItem(dq),this.updateCounter())}),this.updateCounter()}load(){try{return JSON.parse(localStorage.getItem(dq)??`[]`)}catch{return[]}}save(e){if(!this.pad.hasDrawing()){alert(`Сначала нарисуй цифру!`);return}let t=this.preprocessor.process(this.pad.getSvg()),n=Array.from(t,e=>Math.round(e*255)),r=this.load();r.push({label:e,pixels:n});try{localStorage.setItem(dq,JSON.stringify(r))}catch{alert(`Хранилище переполнено — скачай файл и нажми «Сбросить всё»`);return}this.pad.clear(),this.updateCounter()}updateCounter(){let e=this.load(),t=Array(10).fill(0);e.forEach(e=>t[e.label]++),this.counterEl.textContent=`Всего: ${e.length}  [`+t.map((e,t)=>`${t}:${e}`).join(` `)+`]`}download(){let e=this.load();if(e.length===0){alert(`Пока нет примеров`);return}let t=new Blob([JSON.stringify(e)],{type:`application/json`}),n=document.createElement(`a`);n.href=URL.createObjectURL(t),n.download=`user-digits-${e.length}.json`,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(n.href)}};document.addEventListener(`DOMContentLoaded`,()=>{fq.isRequested()?new fq:new sq});